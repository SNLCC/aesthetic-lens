...
① CLI：
   `cd aesthetic-lens && python -m cv_analyzer.cli <图片路径> --pretty --with-image`

② AI 解读：
   从返回 JSON 的 `image_base64` 字段解码图片，用视觉能力看图。
   参考 `known_tags` 和 `cv_classification.traits`，填写 12 字段 JSON：
   ```python
   ai = {
