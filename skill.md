...
> **项目级 Skill** — 安装后数据直接存在 skill 目录下的 ``library/``，跟随安装位置。
>
> ⚠️ 需要 LLM 视觉能力。CV 测量可独立运行，但完整审美分析需要 AI 填充 6 个 ``AI_FILLS`` 字段。
...
1. 运行 `python -m cv_analyzer.cli <图片路径>` 获取 CV 测量 + 初判分类
2. AI 解读图片（需 LLM 视觉能力），**ALL 12 fields must be non-empty**：
